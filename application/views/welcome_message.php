                ...
                if (current_form == "#reg") {
                    if (current_form == "#reg" && $("#reg .pass").val() == $("#reg .retype").val()) {
                        if ($("#reg .name").val() == '' || $("#reg .email").val() == '' || $("#reg .pass").val() == '' || $("#reg .retype").val() == '' || $("#reg .city").val() == '' || $("#reg .house").val() == '' || $("#reg .street").val() == '') {
                            ...
                            $.ajax({
                                method: "GET",
                                url: "<?=base_url()?>welcome/checkemail/" + $("#reg .email").val(),
                    ...
                    if ($("#log .email").val() != '' && $("#log .pass").val() != '') {
                        $.ajax({
                            method: "GET",
                            url: "<?=base_url()?>welcome/checkemailpass/" + $("#log .email").val() + "/" + $("#log .pass").val(),
                            success: function (msg) {
                                if (msg != '1') {
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            function updatecart() {
                $.ajax({
                    method: "GET",
                    url: "<?=base_url()?>welcome/updatecart",
                ...
                $.ajax({
                    method: "GET",
                    url: "<?=base_url()?>welcome/updatecartitems",
                    success: function (msg) {
                        $(".full-cart").html(msg);
                    }
                });
                ...
                $.ajax({
                    method: "GET",
                    url: "<?=base_url()?>welcome/countcart",
                    success: function (msg) {
                        if (msg === '0') {
            ...
            $(".catalog_item_buy").click(function () {
                ...
                $.ajax({
                    method: "GET",
                    url: "<?=base_url()?>welcome/add_cart/" + id,
                    success: function (msg) {
                        if (msg == 1)
            ...
            $(".btn-add-cart").click(function () {
                ...
                $.ajax({
                    method: "GET",
                    url: "<?=base_url()?>welcome/addcart/" + id + "/" + 1,
                    success: function (msg) {
                        if (msg === '1') {
                            updatecart();
                        }
    ...
    <div class="menu">
        ...
        <div class="order-panel" style="display:none;">
            <div class="block-order">
                <div class="wrapper">
                    <span><a href="#" class="log-button" style="color:#a71331;">Войти</a> / <a href="#"
                                                                                               class="reg-button">Регистрация</a></span>
                    <form class="reg-form" id="reg" method="post" action="<?= base_url() ?>welcome/buy">
                        <div class="form-item">
                            <label>имя*</label>
                    ...
                    <form class="log-form" id="log" method="post" action="<?= base_url() ?>welcome/buy"
                          style="display:none;">
                        <div class="form-item">
                            <label>email</label>
...
<div class="main">
            ...
            <? foreach ($sliders as $key => $slider) { ?>
                <div class="item">
                    ...
                    <div class="slider-info">
                        ...
                        <a class="buy btn-add-cart" href="#"
                           value="<?= $slider['product']['product_id'] ?>">Заказать</a>
                    </div>
                </div>
            <? } ?>
        </div>
        ...
        <div class="catalog" id="catalog">
            <div class="row-catalog">
                <? foreach ($products as $key => $product) { ?>
                    <? $zindex = $key * 4; ?>
                    <div style="position:relative;z-index:<?= $zindex ?>" class="product-item <? $check = $key + 1;
                    if ($check % 3 == 0 && $key != 0) echo('last'); ?>" style="margin-bottom:50px;">
                        ...
                        <div class="burger-info data-show-<?= $product['product_id'] ?>"
                        ...
                        <div class="product-info">
                            ...
                            <button class="buy btn-add-cart" value=<?= $product['product_id'] ?> href="#header">
                                Заказать
                            </button>
                        </div>
                    </div>
            ...
            <div class="meat-info wow fadeInRight" data-wow-offset="200" id="meat-info">
                ...
                <p>Порода мясного направления родом из Шотландии. Из мяса получается нежнейший фарш, который мы
                    используем в наших котлетах, а это 160 г чистой мраморной говядины!</p>
            </div>
        </div>
    </div>
    ...
    <div class="delivery wow fadeIn" data-wow-duration="10s" id="delivery">
        <div class="content-del">
            <div class="city">
                <p style="padding-left:60px;">Киев</p>
                <p style="padding-left:120px;">Днепр</p>
                <p style="padding-left:180px;">Одеса</p>
                <p style="padding-left:245px;">Харьков</p>
